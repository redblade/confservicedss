...
/**
 * REST controller for managing {@link eu.pledgerproject.confservice.domain.Project}.
 */
@RestController
@RequestMapping("/api")
public class ProjectResource {
     ...
     */
    @GetMapping("/projects")
    public ResponseEntity<List<Project>> getAllProjects(Pageable pageable, HttpServletRequest request) {
        log.debug("REST request to get a page of Projects");
        SecurityContext securityContext = SecurityContextHolder.getContext();
        if(securityContext.getAuthentication().getAuthorities().iterator().next().getAuthority().equals("ROLE_ROAPI")) {
	        AuditEvent auditEvent = new AuditEvent(((org.springframework.security.core.userdetails.User)securityContext.getAuthentication().getPrincipal()).getUsername(), "READ_PROJECT from IP: " + request.getRemoteAddr());
    ...
    public ResponseEntity<Project> getProject(@PathVariable Long id, HttpServletRequest request) {
